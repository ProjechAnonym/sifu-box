...
func DeleteTemplate(name []string, work_dir string, ent_client *ent.Client, logger *zap.Logger) []gin.H {
	...
	for _, n := range name {
		...
		// 删除模板对应的配置文件（如果存在）
		config_file_path := filepath.Join(work_dir, "temp", "config", fmt.Sprintf(`%s.json`, fmt.Sprintf(`%x`, md5.Sum([]byte(template_msg.Name)))))
...
func FetchYacd(ent_client *ent.Client, bunt_client *buntdb.DB, logger *zap.Logger) (*model.Yacd, error) {
	yacd := model.Yacd{Url: "", Secret: "", Log: false, Template: ""}
	name, err := utils.GetValue(bunt_client, initial.ACTIVE_TEMPLATE, logger)
	if err != nil {
		logger.Error(fmt.Sprintf("获取当前模板信息失败: [%s]", err.Error()))
		return nil, fmt.Errorf("获取当前模板信息失败: [%s]", err.Error())
	}
	yacd.Template = name
	template_msg, err := ent_client.Template.Query().Where(template.NameEQ(name)).Select(template.FieldExperiment, template.FieldLog).First(context.Background())
	if err != nil {
		logger.Error(fmt.Sprintf("获取模板信息失败: [%s]", err.Error()))
		return nil, fmt.Errorf("获取模板信息失败: [%s]", err.Error())
	}
	if template_msg.Experiment.Clash_api.External_controller != "" {
		if !strings.HasPrefix(template_msg.Experiment.Clash_api.External_controller, "http://") && !strings.HasPrefix(template_msg.Experiment.Clash_api.External_controller, "https://") {
			yacd.Url = "http://" + template_msg.Experiment.Clash_api.External_controller
		}
	}
