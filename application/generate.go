...
func (c *Config) Generate(work_dir string, template *ent.Template, outbound_map map[string][]map[string]any, logger *zap.Logger) error {
	...
	if err := utils.WriteFile(path.Join(work_dir, "temp", "config", fmt.Sprintf(`%s.json`, fmt.Sprintf("%x", md5.Sum([]byte(template.Name))))), content, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644); err != nil {
