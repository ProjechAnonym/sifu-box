...
func bootService(logger *zap.Logger, dir string, pid *int, exit *bool, bunt_client *buntdb.DB) {
	defer func() {
		*exit = true
		*pid = 0
	}()
...
func stopService(pid *int, bunt_client *buntdb.DB, logger *zap.Logger) {
	if *pid <= 0 {
...
func ServiceControl(operation *chan int, logger *zap.Logger, dir string, buntdb_client *buntdb.DB, status_chan *chan bool) {
	...
	for signal := range *operation {
		...
		case BOOT_SERVICE:
			...
			exit = false
			go bootService(logger, dir, &singbox_pid, &exit, buntdb_client)
