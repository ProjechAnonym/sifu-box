...
func fetchFromRemote(name, url string, client *http.Client, logger *zap.Logger) ([]map[string]any, error) {
	...
	if err != nil {
		logger.Error(fmt.Sprintf(`"%s"出错,发送请求失败: [%s]`, name, err.Error()))
	...
	if res.StatusCode == 200 {
		...
		if err != nil {
			logger.Error(fmt.Sprintf(`读取"%s"响应失败: [%s]`, name, err.Error()))
		...
		if err := yaml.Unmarshal(content, &config); err == nil {
			...
			if err != nil {
				logger.Error(fmt.Sprintf(`生成"%s"出站节点失败: [%s]`, name, err.Error()))
...
func fetchFromLocal(name, path string, logger *zap.Logger) ([]map[string]any, error) {
	...
	config := map[string]any{}
	if err := yaml.Unmarshal(content, &config); err != nil {
		logger.Error(fmt.Sprintf(`"%s"响应内容不是有效的YAML格式: [%s]`, name, err.Error()))
...
func generateFromYaml(content map[string]any, logger *zap.Logger) ([]map[string]interface{}, error) {
	...
	if !ok {
		logger.Error(fmt.Sprintf(`"%s"字段丢失或不正确`, PROXIES_FIELD))
...
func generateFromBase64(content []byte, logger *zap.Logger) ([]map[string]interface{}, error) {
	...
	if err != nil {
		logger.Error(fmt.Sprintf(`Base64解码失败: [%s]`, err.Error()))
		return nil, err
	}
	var outbounds []map[string]any
