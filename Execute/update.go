...
func Backup_file(origin, backup string,perm fs.FileMode,localhost bool) error{
    // 当备份应在本地主机上执行时
    if localhost{
        // 尝试复制源文件到备份位置
        // 如果复制失败,记录错误并返回
    ...
    // 如果 localhost 为 false 或备份过程成功,返回 nil 表示没有错误
    return nil
}
...
func Update_file(origin_file, new_file, backupfile string, perm fs.FileMode, server database.Server) error{
    // 如果服务器是本地主机
    if server.Localhost{
        ...
        // 备份原文件
        if err := Backup_file(origin_file,backupfile,0644,server.Localhost); err != nil {
